...
2. Pin the dependency to EXACTLY the fixed version — no `^`, `~`, `>=`, or other ranges.
   Prefer: `npm install <package>@<fixedVersion> --save-exact`
   Confirm package.json shows `"<package>": "<fixedVersion>"` (exact string match).
   NEVER leave npm's default caret/tilde save prefix — that fails verification.
...
3. Run the full test suite. If failures:
   ...
   d. If a failure encodes a business rule that the upgrade genuinely violates, STOP:
      ...
      This is a FAILED remediation — do NOT open a pull request and do NOT treat the
      task as successfully completed.
      Forbidden paper-overs for BILL-* / SLA contract tests include:
      - crypto.createSecretKey / HMAC-from-PEM / Buffer.from(pem) as an HS256 secret
      - allowInvalidAsymmetricKeyTypes (or any flag that re-enables rejected key types)
      - rewriting jwt.verify to accept a KeyObject derived from an asymmetric PEM
      - deleting, renaming, or gutting the BILL-* structural/behavioral asserts
...
5. PR body MUST contain: advisory IDs; old → new version; every code change with one-line
   ...
   (Skip this step entirely when blocked per 3d — no PR.)
